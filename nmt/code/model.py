...
class WordPredictor(nn.Module):
    ...
    def __init__(self, inputDim, outputDim, ignoreIndex=-1):
        ...
        self.loss = nn.CrossEntropyLoss(
            size_average=False, ignore_index=ignoreIndex)
...
class EncDec(nn.Module):
    ...
    def forward(self, inputTarget, lengthsTarget, lengthsSource, hidden0Target, sourceH, target=None):
        ...
        finalHidden = Variable(inputTarget.data.new(
            batchSize, maxLen, self.targetEmbedDim), requires_grad=False)
        prevFinalHidden = Variable(inputTarget.data.new(
            batchSize, 1, self.targetEmbedDim).zero_(), requires_grad=False)
        ...
        for i in range(maxLen):
            ...
            attentionScores = attentionScores_.data.new(
            ...
            attentionScores = F.softmax(
                attentionScores.transpose(0, 2)).transpose(0, 2)
    ...
    def greedyTrans(self, bosIndex, eosIndex, lengthsSource, targetEmbedding,
                    sourceH, hidden0Target, maxGenLen=100, return_attention=False):
        batchSize = sourceH.size(0)
        i = 1
        eosCount = 0
        targetWordIndices = Variable(torch.LongTensor(
            batchSize, maxGenLen).fill_(bosIndex), requires_grad=False).cuda()
        ...
        prevFinalHidden = Variable(sourceH.data.new(
            batchSize, 1, self.targetEmbedDim).zero_(), requires_grad=False)
        ...
        if testMode == True:
            return targetWordIndices, list(targetWordLengths), attentionIndices, attentionScores
